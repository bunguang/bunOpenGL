//
// Created by bunguang on 2018/11/9.
//
...
int drawWithInstancing();

int drawWithoutInstancing();
...
int drawWithInstancing() {
    ...
    Shader planetShader("simple.vert", "simple.frag");
    Shader rockShader("instancing.vert", "instancing.frag");
    ...
    for (unsigned int i = 0; i < INSTANCE_AMOUNT; i++) {
        ...
        float angle = (float) i / (float) INSTANCE_AMOUNT * 360.0f;
    ...
    glBufferData(GL_ARRAY_BUFFER, INSTANCE_AMOUNT * sizeof(glm::mat4), &modelMatrices[0], GL_STATIC_DRAW);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        for (unsigned int i = 0; i < rock.meshes.size(); i++)
        {
            ...
            glDrawElementsInstanced(GL_TRIANGLES, rock.meshes[i].indices.size(), GL_UNSIGNED_INT, 0, INSTANCE_AMOUNT);
            glBindVertexArray(0);
        }
